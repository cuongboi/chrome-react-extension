...
import { cn, deepEquals } from '@/lib/utils';
...
interface MultipleSelectorProps {
  ...
  optionLabelRender?: (option: Option) => React.ReactNode;
}
...
const MultipleSelector = ({
  ...
  useEffect(() => {
    ...
    if (!deepEquals(newOption, options)) {
        ...
        <div className="flex flex-wrap gap-1">
          {selected.map((option) => {
            return (
              ...
              >
                {typeof optionLabelRender === 'function'
                  ? optionLabelRender(option)
                  : option.label}
                <button
                  className="text-muted-foreground/80 hover:text-foreground focus-visible:border-ring focus-visible:ring-ring/50 absolute -inset-y-px -end-px flex size-7 items-center justify-center rounded-e-md border-none p-0 outline-hidden transition-[color,box-shadow] outline-none focus-visible:ring-[3px]"
                  onKeyDown={(e) => {
                    if (e.key === 'Enter') {
