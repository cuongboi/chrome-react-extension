...
export default function ColumnMap() {
  ...
  const columnOptions = React.useMemo(() => {
    return {
      ...
      group: Object.entries(columns).map(
        ([key, value]) =>
          value.dataType === 'iteration' && (
            <SelectItem value={key} key={key}>
              {value.name}
            </SelectItem>
          ),
              ...
              >
                <FormControl>
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder="Select as Group column" />
                  </SelectTrigger>
                </FormControl>
                <SelectContent>{columnOptions.group}</SelectContent>
              </Select>
