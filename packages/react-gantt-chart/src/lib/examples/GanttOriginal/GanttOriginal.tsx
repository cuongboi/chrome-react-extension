...
const GanttOriginal = (props: IProps & typeof defaultProps) => {
  ...
  useEffect(() => {
    if (tasks.length > 0) {
      const [startDate] = ganttDateRange(tasks, viewMode);
      const newDates = seedDates(startDate, new Date(), viewMode);

      if (newDates.length - spintDuration > 0) {
        setScrollX((newDates.length - spintDuration) * columnWidth);
      }
    }
  }, [tasks]);
